...
function retrieveAbout () {
	...
	if ($type == 'committee') {
		...
		$eventsQuery = array( 'post_type' => 'events', 'category_name' => $category, 'posts_per_page' => 1, 'meta_key' => 'events-date', 'orderby' => 'meta_value', 'order' => 'DESC');
		$eventsLoop = new WP_Query( $eventsQuery );

		while ($eventsLoop->have_posts()) {
			$eventsLoop->the_post();
			global $post;
			$eventDate = get_post_meta($post->ID, 'events-date', true);
